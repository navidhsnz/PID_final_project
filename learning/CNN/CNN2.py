...
# ii=0
# while True:
#     image = cv2.imread(f"../datasets/trail2/images/image_{ii}.jpg")
...
def train(model, dataloader, val_loader, criterion, optimizer, epochs, snapshot_epochs):
    ...
    for epoch in range(epochs):
        ...
        if epoch in snapshot_epochs:
            checkpoint_path = f"lane_detection_rnn_new_CNN2_{epoch}.pth"
...
# this is the training part of the code. to trian the model, simply remove the 2 from "__main__2", and run the code.
if __name__ == "__main__":

    transform = get_transform()

    train_loader, val_loader, test_loader = loader_creator(
        image_dir="../datasets/trail2/images",
        label_dir="../datasets/trail2/labels",
        transform=transform,
        batch_size=16,
        train_split=0.8,
        val_split=0.2
    ...
    # train model
    train_losses, val_losses = train(model, train_loader, val_loader, criterion, optimizer, epochs_num, snapshot_epochs=[5,10,20])

    # save the model
    torch.save(model.state_dict(), "lane_detection_CNN2.pth")

    with open("train_losses_CNN2.txt", "w") as train_file:
        for loss in train_losses:
            train_file.write(f"{loss}\n")

    with open("val_losses_CNN2.txt", "w") as val_file:
        for loss in val_losses:
            val_file.write(f"{loss}\n")

    print("Train and validation losses saved to 'train_losses_CNN2.txt' and 'val_losses_CNN2.txt'")
