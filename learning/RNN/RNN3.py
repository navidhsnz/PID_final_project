...
class LaneDetectionCNN(nn.Module):
    def __init__(self, input_shape=(3, 224, 224)):
        ...
        # self.fc1 = nn.Linear(self._to_linear, 200) # 7x7 image with 3 channels
...
def train_model_with_checkpoints(model, train_loader, val_loader, criterion, optimizer, device, n_epochs, save_epochs):
    ...
    for epoch in range(n_epochs):
        ...
        if epoch in save_epochs:
            checkpoint_path = f"lane_detection_rnn_new_RNN3_{epoch}.pth"
...
if __name__ == "__main__":
    ...
    with open("train_losses_RNN3.txt", "w") as train_file:
        for loss in train_losses:
            train_file.write(f"{loss}\n")

    with open("val_losses_RNN3.txt", "w") as val_file:
        for loss in val_losses:
            val_file.write(f"{loss}\n")

    print("Train and validation losses saved to 'train_losses_RNN3.txt' and 'val_losses_RNN3.txt'")
